...
@click.option(
    ...
    type=str,
    default=None,
    help="Pre-set profile: best, maximum, mild, code, system, output, mcp",
)
@click.option(
    "--temperature-initial", type=float, default=None, help="Initial quenching temperature T0"
)
@click.option("--cooling-rate", "--alpha", type=float, default=None, help="Quenching rate alpha")
@click.option(
    "--similarity-threshold", type=float, default=None, help="Fidelity safety threshold [0, 1]"
)
@click.option(
    "--w-statistical", type=float, default=None, help="Weight for statistical energy component"
)
@click.option(
    "--w-structural", type=float, default=None, help="Weight for structural energy component"
)
@click.option(
    "--w-positional", type=float, default=None, help="Weight for positional energy component"
)
@click.option(
    "--output-cooling/--no-output-cooling", default=None, help="Enable output-side cooling"
)
@click.option("--dedup/--no-dedup", default=None, help="Enable context dedup pre-pass")
@click.option(
    "--memory/--no-memory", default=None, help="Enable cross-request memory-aware compression"
)
@click.option(
    "--multi-turn/--no-multi-turn", default=None, help="Enable structural multi-turn compression"
)
@click.option(
    "--turn-decay", type=float, default=None, help="Per-turn temperature decay multiplier (0,1]"
)
@click.option(
    "--protected-recent-turns", type=int, default=None, help="Last N turns left at full temperature"
)
...
def main(
    ...
    if profile:
        ...
        if temperature_initial is None:
            cc.temperature_initial = pc.temperature_initial
        if cooling_rate is None:
            cc.cooling_rate = pc.cooling_rate
        if similarity_threshold is None:
            cc.similarity_threshold = pc.similarity_threshold
        if w_statistical is None:
            cc.energy_weights.w_statistical = pc.energy_weights.w_statistical
        if w_structural is None:
            cc.energy_weights.w_structural = pc.energy_weights.w_structural
        if w_positional is None:
            cc.energy_weights.w_positional = pc.energy_weights.w_positional
        if output_cooling is None:
            cc.output_cooling = pc.output_cooling
        if dedup is None:
            cc.dedup_enabled = pc.dedup_enabled
        if memory is None:
            cc.memory_enabled = pc.memory_enabled

    if w_statistical is not None:
        cc.energy_weights.w_statistical = w_statistical
    if w_structural is not None:
        cc.energy_weights.w_structural = w_structural
    if w_positional is not None:
        cc.energy_weights.w_positional = w_positional
    ...
    click.echo(
        f"  Weights:     stat={cc.energy_weights.w_statistical}, "
